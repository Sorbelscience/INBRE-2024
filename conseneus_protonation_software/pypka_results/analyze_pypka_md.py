...
# Load the modified PDB and DCD files from pyPKa results
u = mda.Universe('k1_pypka.pdb', 'pypka_trajectory.dcd')
...
# Protonation states data (manually provided, updated with pyPKa data)
protonation_states = {
    ('GLU', 'A', 45): 3.84,
    ('NTR', 'A', 45): 8.11,
    ('TYR', 'A', 49): 10.52,
...
def plot_rmsd(rmsd, interval_ns):
    ...
    plt.xlabel('Time (ns)')
    plt.ylabel('RMSD (Å)')
    plt.title('RMSD Over Time for pyPKa')
    plt.legend()
    plt.savefig(f'pyPKa_rmsd_dark_{interval_ns}ns_{timestamp}.png')
    plt.close()
...
def plot_radius_of_gyration(Rg, interval_ns):
    ...
    plt.title('Radius of Gyration Over Time for pyPKa')
    plt.savefig(f'pyPKa_radius_of_gyration_dark_{interval_ns}ns_{timestamp}.png')
    plt.close()
...
def plot_hydrogen_bonds(hbond_counts, interval_ns):
    ...
    plt.title('Hydrogen Bonds Over Time for pyPKa')
    plt.savefig(f'pyPKa_hydrogen_bonds_dark_{interval_ns}ns_{timestamp}.png')
    plt.close()
...
def plot_rmsf(rmsf):
    ...
    plt.xlabel('Residue')
    plt.ylabel('RMSF (Å)')
    plt.title('RMSF for pyPKa')
    plt.legend()
    plt.savefig(f'pyPKa_rmsf_dark_{timestamp}.png')
    plt.close()
