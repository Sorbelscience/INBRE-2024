...
def plot_rmsd(rmsd, interval_ns):
    ...
    for i in range(0, len(time), len(time) // 10):  # Annotate every 10th point
        plt.text(time[i], rmsd[i, 2], f'{rmsd[i, 2]:.2f}', fontsize=8, ha='center')
    plt.xlabel('Time (ns)')
    plt.ylabel('RMSD (Å)')
    plt.title('RMSD Over Time for H++')
    plt.legend()
    plt.savefig(f'H++_rmsd_dark_{interval_ns}ns.png')
...
def plot_radius_of_gyration(Rg, interval_ns):
    ...
    plt.savefig(f'H++_radius_of_gyration_dark_{interval_ns}ns.png')
...
def plot_hydrogen_bonds(hbond_counts, interval_ns):
    ...
    plt.savefig(f'H++_hydrogen_bonds_dark_{interval_ns}ns.png')
...
def plot_rmsf(rmsf):
    ...
    for i in range(0, len(residues), len(residues) // 10):  # Annotate every 10th residue
        plt.text(residues[i], rmsf[i], f'{rmsf[i]:.2f}', fontsize=8, ha='center')
    plt.xlabel('Residue')
    plt.ylabel('RMSF (Å)')
    plt.title('RMSF for H++')
    plt.legend()
    plt.savefig('H++_rmsf_dark.png')
...
def main(interval_ns=1):
    ...
    print(f"Analysis complete. Plots saved as 'H++_rmsd_dark_{interval_ns}ns.png', 'H++_radius_of_gyration_dark_{interval_ns}ns.png', 'H++_hydrogen_bonds_dark_{interval_ns}ns.png', and 'H++_rmsf_dark.png'.")
