...
def plot_rmsd(rmsd, interval_ns):
    ...
    plt.figure()
    plt.plot(time, rmsd[:, 2], color='cyan', label='RMSD')
    ...
    plt.legend()
    plt.savefig(f'H++_rmsd_dark_{interval_ns}frames_{timestamp}.png')
    plt.close()
...
def plot_radius_of_gyration(Rg, interval_ns):
    ...
    plt.figure()
    plt.plot(time, Rg, color='cyan', label='Radius of Gyration')
    step = max(1, len(time) // 10)  # Ensure the step is at least 1
    for i in range(0, len(time), step):  # Annotate every 10th point or more
    ...
    plt.legend()
    plt.savefig(f'H++_radius_of_gyration_dark_{interval_ns}frames_{timestamp}.png')
    plt.close()
...
def plot_hydrogen_bonds(hbond_counts, interval_ns):
    ...
    plt.figure()
    plt.plot(time, hbond_counts, color='cyan', label='Hydrogen Bonds')
    step = max(1, len(time) // 10)  # Ensure the step is at least 1
    for i in range(0, len(time), step):  # Annotate every 10th point or more
    ...
    plt.legend()
    plt.savefig(f'H++_hydrogen_bonds_dark_{interval_ns}frames_{timestamp}.png')
    plt.close()
...
def main(interval_ns=1):
    ...
    print(f"Analysis complete. Plots saved as 'H++_rmsd_dark_{interval_ns}frames_{timestamp}.png', 'H++_radius_of_gyration_dark_{interval_ns}frames_{timestamp}.png', 'H++_hydrogen_bonds_dark_{interval_ns}frames_{timestamp}.png', and 'H++_rmsf_dark_{timestamp}.png'.")
